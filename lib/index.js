...
function cdf( x, options ) {
	...
	if ( isMatrixLike( x ) ) {
		...
		return cdf5( out, x, opts.mu, opts.b );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.mu, opts.b );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.mu, opts.b, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.mu, opts.b, opts.accessor );
		}
		return cdf2( out, x, opts.mu, opts.b );
	}
	return NaN;
} // end FUNCTION cdf()
