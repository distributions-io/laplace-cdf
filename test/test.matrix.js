...
describe( 'matrix cdf', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			cdf( matrix( [10,10] ), mat, mu, b );
		}
	});

	it( 'should evaluate the Laplace cdf for each matrix element', function test() {
		var actual, i;

		actual = matrix( [5,5], 'float64' );
		actual = cdf( actual, mat, mu, b );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( cdf( out, mat, mu, b ).data, expected );
